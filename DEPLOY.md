...
```
用户浏览器 ──HTTPS──► Nginx (反向代理) ──► Gradio App (:7860)
                                              │
                                    预建好的人脸索引 (cache/)
                                    ONNX 模型 (models/)
```
...
| 方案 | 配置 | 月费参考 | 适用场景 |
|------|------|----------|----------|
| **CPU 云服务器（推荐）** | 4 核 8G，Ubuntu 22.04 | ¥100-300 | 几百次搜索，索引已预建 |
| GPU 云服务器 | T4/A10，16G+ | ¥2000-5000 | 需要现场实时建索引 |

> 索引预建好后，每次搜索只做一次人脸检测（~1-2s CPU）+ 向量比对（<0.1s），
> **普通 CPU 服务器完全够用**。

推荐：阿里云/腾讯云轻量应用服务器，4 核 8G，按量付费或包月。
...
1. 你把链接 `https://photo.你的域名.com` 和访问码发到活动群
2. 用户打开链接 → 输入访问码（用户名随意）→ 进入搜索页
3. 相册链接已预填，用户只需**拍一张自拍或上传正脸照**
4. 点击「开始查找」→ 几秒后看到结果
5. 点击「打包下载」获取 zip
