...
- **识别**: ArcFace `w600k_r50` (512-d embedding, 余弦相似度)
...
粘贴相册链接 → 上传参考人脸照片 → 点击"开始查找"。
结果以卡片墙展示：缩略图预览、相似度、文件名，并可跳转到**活动原网页**或**原图直链**。
...
- 参考照片请使用清晰正脸照；侧脸/低头/遮挡会显著降低召回。
- 支持带密码的相册（`--pwd`）；其他类似平台可参照 `crawler.py` 适配 API。
