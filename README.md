...
> 适用于任何**一拍即传 (yipai360)** 活动相册。粘贴相册链接 + 上传一张自拍照，
> 自动在成百上千张活动照片中找出你出现的所有瞬间。

输入活动相册链接 + 一张参考人脸照片，系统自动遍历相册中的全部照片（含瀑布流/懒加载内容），
缓存图片并建立人脸索引，通过人脸相似度检索找出目标人物出现的所有照片，
以**图片预览 + 大图链接**形式返回结果。

> **关于预填链接**：Web UI 中预填的相册链接仅为示例，使用时替换为你自己的活动相册链接即可。
> 同一套部署可服务不同活动——只需在搜索时粘贴对应的相册 URL。

🔗 **在线体验**：http://47.76.47.229:7860/
...
1. 粘贴活动相册链接（预填链接仅为示例，替换为你自己的活动链接）
2. 在"添加参考照片"区域上传照片（可反复添加多张正脸/侧脸）
   - 添加后自动检测人脸并反馈质量（是否清晰、是否有多张人脸）
   - 支持"撤销最后一张"和"清空全部"
3. 点击"开始查找"，搜索过程中可随时点"✕ 取消"
4. 结果以卡片墙展示：缩略图预览（**绿框标注命中人脸**）、相似度、文件名
5. 有命中结果时出现"📦 打包下载全部命中照片"按钮，可一键导出 zip
...
- 目标站点无独立"单张照片页"，结果顶部的"打开活动原网页"链接指向相册页，配合文件名/photoId 定位。
